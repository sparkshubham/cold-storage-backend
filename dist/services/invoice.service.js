...
export function storageDaysBetween(from, to) {
...
async function loadSettings(companyId) {
    return ((await SettingsModel.findOne({ companyId })) ?? {
        invoicePrefix: 'INV',
        defaultGstRate: 18,
...
async function existingIssuedInvoice(companyId, sourceType, sourceId) {
...
export async function buildInvoiceDraft(companyId, sourceType, sourceId, rates = {}) {
    ...
    const relatedInward = sourceType === 'inward' ? source : await findRelatedInward(source);
    ...
    const sourceInvoiceId = idOf(source.invoiceId);
    ...
    return {
        ...
        sourceNumber: sourceType === 'inward' ? source.inwardNumber : source.outwardNumber,
        quantity,
        unit,
...
export async function previewInvoice(companyId, sourceType, sourceId, rates = {}) {
    return buildInvoiceDraft(companyId, sourceType, sourceId, rates);
}
export async function generateInvoice(companyId, input, actor) {
...
export async function listInvoices(companyId, params) {

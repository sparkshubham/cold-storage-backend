...
export async function logout(refreshToken, userId, ip, userAgent) {
    const hash = hashToken(refreshToken);
    await RefreshTokenModel.updateMany({ userId, tokenHash: hash, revokedAt: null }, { $set: { revokedAt: new Date() } });
    await writeAudit({
        userId,
        action: 'LOGOUT',
        module: 'Auth',

...
function mapStockError(err, inbound) {
    const message = err instanceof Error ? err.message : String(err);
    if (message === 'NEGATIVE_QUANTITY')
        throw AppError.badRequest('Insufficient stock for this movement');
    if (message === 'NEGATIVE_OCCUPANCY')
        throw AppError.badRequest('Occupancy cannot be negative');
...
async function applyOccupancy(companyId, locationId, delta, session) {
    ...
    if (!rack || !chamber)
        throw AppError.notFound('Storage location is incomplete');
    try {
...
export async function applyStockMovement(input, session) {
    ...
    if (!inventory) {
        if (delta < 0)
            throw AppError.badRequest('No stock exists at this location');
    ...
    inventory.updatedBy = input.actor.id;
    await inventory.save({ session });
    await applyOccupancy(input.companyId, input.locationId, delta, session);
    const [transaction] = await createDocs(StockTransactionModel, [
        {
            companyId: input.companyId,
            type: input.type,
            customerId: input.customerId,
...
async function assertMasters(companyId, customerId, productId) {
...
export async function listInventory(companyId, params) {
...
export async function listStockTransactions(companyId, params) {
...
async function maybeCreateBatch(companyId, input, actor, inwardDate, session) {
    if (!input.batchNumber)
        return input.batchId ?? null;
    const [batch] = await createDocs(BatchModel, [
        {
            companyId,
...
export async function createOpeningStock(companyId, input, actor) {
...
export async function getInward(companyId, id) {
    const doc = await InwardModel.findOne({ _id: id, companyId, deletedAt: null }).populate(movementPopulate);
    if (!doc)
        throw AppError.notFound('Inward not found');
    return doc;
}
export async function getOutward(companyId, id) {
    const doc = await OutwardModel.findOne({ _id: id, companyId, deletedAt: null }).populate(movementPopulate);
    if (!doc)
        throw AppError.notFound('Outward not found');
    return doc;
}
export async function listInwards(companyId, params) {
    const filter = { companyId, deletedAt: null };
    if (params.search) {
        const rx = new RegExp(escapeRegex(params.search), 'i');
...
export async function createInward(companyId, input, actor) {
    await assertMasters(companyId, String(input.customerId), String(input.productId));
    const inwardNumber = await nextCode(InwardModel, companyId, 'INW', 'inwardNumber');
    return withTransaction(async (session) => {
        const [inward] = await createDocs(InwardModel, [
            {
                customerId: input.customerId,
                productId: input.productId,
                quantity: Number(input.quantity),
...
export async function listOutwards(companyId, params) {
    const filter = { companyId, deletedAt: null };
    if (params.search) {
        const rx = new RegExp(escapeRegex(params.search), 'i');
...
export async function createOutward(companyId, input, actor) {
    await assertMasters(companyId, String(input.customerId), String(input.productId));
    const outwardNumber = await nextCode(OutwardModel, companyId, 'OUT', 'outwardNumber');
    return withTransaction(async (session) => {
        const [outward] = await createDocs(OutwardModel, [
            {
                customerId: input.customerId,
                productId: input.productId,
                quantity: Number(input.quantity),
...
export async function createAdjustment(companyId, input, actor) {
    await assertMasters(companyId, String(input.customerId), String(input.productId));
    const quantity = Number(input.quantity);
    if (!quantity)
        throw AppError.badRequest('Adjustment quantity cannot be zero');
    return withTransaction(async (session) => applyStockMovement({
        companyId,

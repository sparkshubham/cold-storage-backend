...
async function seedPlatformRole() {
    ...
    await RoleModel.updateOne({ code: superAdmin.code, companyId: null }, {
        $set: {
...
async function seedDemoCompany() {
    ...
    const company = await CompanyModel.findOneAndUpdate({ email: 'demo@abccold.test' }, {
        $set: {
...
async function seedOperationalData() {
    ...
    const peas = await productService.create(companyId, { name: 'Frozen Peas', code: 'PEAS', categoryId: veg._id, unitId: mt._id, storageType: 'Frozen' }, actor);
    await productService.create(companyId, { name: 'Butter', code: 'BUTTER', categoryId: veg._id, unitId: kg._id, storageType: 'Chilled' }, actor);
    const customer = await customerService.create(companyId, { name: 'FreshMart Traders', mobile: '9811111111', city: 'Agra', state: 'Uttar Pradesh' }, actor);
    ...
    await createOpeningStock(companyId, {
